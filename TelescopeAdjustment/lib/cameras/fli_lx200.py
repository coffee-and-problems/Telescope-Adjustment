...
camera = Camera("fli_lx200")


def parse_object_file_name(fName):
    """ Function gets object name, add string (if exists) and
    filter name out of object file name"""
    # File name is like objname+[addstr]+filter[s]+S+number.FIT,
    # for example for wcom it may be 'wcom2bS001.FIT'
    ...
    # Now we have only filter and frame number in our string. It looks like rS12 now.
    sIdx = filterAndFrameNumber.index("S")

    # filter combination
    filterCombination = filterAndFrameNumber[:sIdx]
...
def match_files(pathToData, objName, addString, filterCombination):
    ...
    fileNamePattern1 = objName + addString + filterCombination + "S*.fit"
    fileNamePattern2 = objName + addString + filterCombination[-1::-1] + "S*.fit"
